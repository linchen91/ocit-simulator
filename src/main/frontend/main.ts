import { Api, TrafficSignalController, SystemStatus, SignalGroup } from './api.js';

const DATEX2_COLORS: Record<string, string> = {
  normal: '#22c55e',
  minor: '#eab308',
  major: '#f97316',
  severe: '#ef4444'
...
const OCITC_COLORS: Record<string, string> = {
  SUCCESS: '#22c55e',
  FAILURE: '#ef4444',
  PENDING: '#eab308',
  TIMEOUT: '#6b7280'
...
let datex2RefreshInterval: number | null = null;
...
async function loadSystemStatus(): Promise<SystemStatus | null> {
  try {
...
async function showDetails(controllerId: string) {
...
async function setStatus(controllerId: string, status: string) {
  if (!status) return;
  try {
...
function parseXmlDatex2(xml: string): string {
  try {
...
async function loadDatex2Data() {
  ...
  try {
    ...
    panel.innerHTML = `
      ...
      <div class="datex2-card">
        <h4>Situation</h4>
        <p>Severity: <span style="color: ${DATEX2_COLORS[severity] || '#6b7280'}">${severity.toUpperCase()}</span></p>
        <p>Location: <span>${lat.toFixed(4)}, ${lon.toFixed(4)}</span></p>
      </div>
      <div class="datex2-card">
        <h4>Traffic Data</h4>
        <p>Speed: <span>${speed.toFixed(1)} km/h</span></p>
        <p>Vehicles: <span>${vehicles}</span></p>
      </div>
      <div class="datex2-card">
        <h4>System Status</h4>
        <p>Status: <span style="color: ${status === 'operational' ? '#22c55e' : '#ef4444'}">${status.toUpperCase()}</span></p>
        <p>Updated: <span>${new Date().toLocaleTimeString()}</span></p>
      </div>
    `;
  } catch (err) {
    console.error('Failed to load Datex2 data:', err);
    if (panel) panel.innerHTML = '<p>Failed to load DATEX II data</p>';
  }
}
  ...
  try {
    ...
    try {
      ...
      const activeEl = 
        ...
        doc.getElementsByTagName('ACTIVE')[0] ||
        null;
      cycleTime = cycleEl?.textContent ? parseInt(cycleEl.textContent.trim()) : 0;
      active = activeEl?.textContent?.trim() === 'true';
    } catch {}
    ...
    panel.innerHTML = `
      <div class="ocitc-card">
        ...
        <p>Message: <span>${msgType.toUpperCase()}</span></p>
      </div>
      <div class="ocitc-card">
        <h4>Supply Data</h4>
        <p>Type: <span>${objectType.toUpperCase()}</span></p>
        <p>Manufacturer: <span>${manufacturer}</span></p>
        <p>Status: <span style="color: ${STATUS_COLORS[statusC] || '#6b7280'}">${statusC}</span></p>
      </div>
      <div class="ocitc-card">
        <h4>Process</h4>
        <p>Cycle: <span>${cycleTime}s</span></p>
    ...
    if (panel) panel.innerHTML = '<p>Failed to load OCIT-C data</p>';
  }
}

function updateStatusPanel(status: SystemStatus | null) {
  const panel = document.getElementById('status-panel');
  if (!panel || !status) return;

  panel.innerHTML = `
    <div class="stat-card">
      <span class="stat-value">${status.totalControllers}</span>
      <span class="stat-label">Total</span>
    </div>
    <div class="stat-card">
      <span class="stat-value">${status.activeControllers}</span>
...
declare global {
  interface Window {
    app: {
      showDetails: typeof showDetails;
